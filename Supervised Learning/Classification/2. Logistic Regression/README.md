...
This Jupyter notebook applies the Logistic Regression algorithm to the titanic dataset PCA first two principal components. We utilize an implementation of a single logistic regression neuron trained on the first two principal components of the data to attempt to predict titanic survivorship. We then plot the data with the line predicted by the neuron and the error of the algorithm. We conclude that the logistic regression neuron models the data reasonably well.

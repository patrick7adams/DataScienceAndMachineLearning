...
This Jupyter notebook applies the Perceptron algorithm to the titanic dataset PCA first two principal components. We utilize an implementation of a single perceptron neuron trained on the first two principal components of the data to attempt to predict titanic survivorship. We then plot the data with the line predicted by the Perceptron and the error of the Perceptron algorithm. We conclude that the Perceptron models the PCA data poorly.
